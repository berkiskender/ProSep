...
def train_theta_eta_simult(model, optimizer_latent, criterion_g, criterion_psi,
                           ...
                           ortho_weight=1e-6, verbose=True, verbose_freq=2):
    # Initialize loss vectors
    min_f_pol_s_epoch = 1e8
    spatial_idx = np.arange(spatial_dim)
    loss_epoch, loss_temporal_epoch, loss_g_epoch, loss_Theta_epoch, loss_f_pol_s_epoch = [], [], [], [], [min_f_pol_s_epoch]
    ...
    # For orthogonality temporal constraint
    ...
    for epoch in range(num_epoch):
        loss_sum, loss_temporal_sum, loss_Theta_sum, loss_g_sum, loss_f_pol_s_sum = 0, 0, 0, 0, 0
        ...
        if training_mode == 'single_view':
            ...
            # Enforce orthonormality for temporal latent fcts. for non/linear approach
            if learning_mode in ['only_temp_rep_learn_linear', 'only_temp_rep_cubic_spline']:
                T = model.temporal_fcts[0, :, :].permute(1,0)
            # Compute temporal orthogonality loss
            ...
            if loss_f_pol_s_epoch[-1] < min_f_pol_s_epoch:
                min_f_pol_s_epoch = copy.deepcopy(loss_f_pol_s_epoch[-1])
                best_model = copy.deepcopy(model)
                best_psi_mtx = psi_mtx.detach().clone()

            if epoch % verbose_freq == 0 and verbose:
    ...
    return best_model, loss_epoch, loss_temporal_epoch, loss_g_epoch, loss_f_pol_s_epoch, loss_Theta_epoch, g_gt, theta_gpu, L1_est, best_psi_mtx
